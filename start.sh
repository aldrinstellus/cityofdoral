...
# ============================================================
# City of Doral - Primary Entry Point Startup Script
# ============================================================
# This script starts all required services for the City of Doral
# website with embedded AI chatbot.
#
# PRIMARY URL: http://localhost:8888/Home/index.html
#
# Services:
#   - Next.js Backend (port 3000): Chat API & widget assets
#   - Static Site Server (port 8888): Scraped website with chatbot
# ============================================================
...
echo -e "${YELLOW}Starting Next.js backend on port 3000...${NC}"
...
echo -e "    - Next.js API:    http://localhost:3000"
...
while true; do
    ...
    if ! kill -0 "$NEXTJS_PID" 2>/dev/null; then
        echo -e "${RED}Next.js server stopped unexpectedly. Check /tmp/nextjs-cityofdoral.log${NC}"
        exit 1
    fi

    if ! kill -0 "$STATIC_PID" 2>/dev/null; then
        echo -e "${RED}Static server stopped unexpectedly. Check /tmp/static-cityofdoral.log${NC}"
        exit 1
    fi
