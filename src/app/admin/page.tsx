...
const handleExportJSON = async () => {
  try {
    const response = await fetch('/api/analytics?days=7&format=json');
    ...
    a.href = url;
    a.download = `doral-analytics-${new Date().toISOString().split('T')[0]}.json`;
...
const handleExportCSV = async () => {
  try {
    const response = await fetch('/api/analytics?days=7&format=csv');
    if (!response.ok) throw new Error('Failed to fetch data');
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `doral-analytics-${new Date().toISOString().split('T')[0]}.csv`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
