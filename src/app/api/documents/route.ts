...
export async function POST(request: NextRequest) {
  try {
    ...
    kb.entries = kb.entries.filter(e => !e.id.startsWith(filePrefix));

    // Add new entries
    kb.entries.push(...entries);
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const kb = await loadKnowledgeBase();
    const originalCount = kb.entries.length;

    // Remove entries from this file
    kb.entries = kb.entries.filter(e => !e.id.startsWith(filePrefix));

    const removedCount = originalCount - kb.entries.length;
