// Multilingual support for City of Doral Chatbot
...
export function getSystemPrompt(
  ...
  if (language === 'ht') {
    return `Ou se Asistan Vyetyel Vil Doral la.
...
KRITIK: Chak pwen = MAKS 6 MO. San eksepsyon.`;
  ...
  if (language === 'es') {
    return `Eres el Asistente Virtual de la Ciudad de Doral.
...
CRÍTICO: Cada viñeta = MÁX 6 PALABRAS. Sin excepciones.`;
  ...
  return `You are the City of Doral Virtual Assistant.
...
SPECIAL RULE FOR CITY COUNCIL MEETINGS:
When asked about City Council meetings or agenda, use this format:
**Next Meeting:**
• Date, time, and location first

**Agenda Highlights:**
• Show up to 5 key agenda items from the context
• End with: "View full agenda: www.cityofdoral.com/Government-Center/Office-of-the-City-Clerk/Council-Meetings"
...
export const UI_LABELS = {
  ...
  ht: {
    ...
    escalateMessage: 'Eske ou ta renmen pale ak yon reprezantan vil la?',
    language: 'Lang',
    english: 'English',
    spanish: 'Español',
    haitianCreole: 'Kreyòl Ayisyen',
    suggestedQuestions: [
      'Ki le City Hall ouvè?',
      'Kijan mwen aplike pou yon pemis konstriksyon?',
      'Ki evènman ki ap vini?',
      'Kote pak yo ye?',
    ],
    welcome: "Bonjou! Mwen se Asistan AI Vil Doral la. Mwen ka ede ou ak enfomasyon sou sevis vil la, pemis, evènman, pak, ak plis ankò. Kijan mwen ka ede ou jodi a?",
  },
};
