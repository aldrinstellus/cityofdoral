...
export function generateGreeting(options: IVRGreetingOptions = {}): string {
  ...
  const greetingEn = `Hello! Welcome to the City of Doral virtual assistant.
    You can ask me questions about city services, permits, parks, and more.
    Press 1 at any time to speak with a representative.
    Press 2 to get a code to continue this conversation on our website.
    How can I help you today?`;

  const greetingEs = `Hola! Bienvenido al asistente virtual de la Ciudad de Doral.
    Puede hacerme preguntas sobre servicios municipales, permisos, parques, y mas.
...
export function generateTransferCode(language: Language, code: string): string {
  ...
  const transferMsgEn = `I will give you a transfer code to continue this conversation on our website.
    ...
    Go to the City of Doral website, open the chat assistant, click the key icon, and enter this code.
  ...
  const transferMsgEs = `Le dare un codigo de transferencia para continuar esta conversacion en nuestro sitio web.
    ...
    Vaya al sitio web de la Ciudad de Doral, abra el asistente de chat, haga clic en el icono de llave, e ingrese este codigo.
