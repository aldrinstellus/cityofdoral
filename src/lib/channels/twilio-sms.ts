...
export function generateOptOutResponse(language: 'en' | 'es'): string {
  const response = new MessagingResponse();

  const message = language === 'es'
    ? 'Ha sido removido de los mensajes de texto de la Ciudad de Doral. Responda START para volver a recibir mensajes.'
    : 'You have been unsubscribed from City of Doral text messages. Reply START to resubscribe.';
...
export function generateOptInResponse(language: 'en' | 'es'): string {
  const response = new MessagingResponse();

  const message = language === 'es'
    ? 'Bienvenido de nuevo! Ahora recibira mensajes de la Ciudad de Doral. Responda STOP para cancelar.'
    : 'Welcome back! You will now receive messages from the City of Doral. Reply STOP to unsubscribe.';
...
export function generateHelpResponse(language: 'en' | 'es'): string {
  const response = new MessagingResponse();

  const message = language === 'es'
    ? 'Ciudad de Doral SMS: Pregunte sobre horarios, permisos, eventos y servicios. Responda STOP para cancelar o llame al 305-593-6700.'
    : 'City of Doral SMS: Ask about hours, permits, events, and services. Reply STOP to unsubscribe or call 305-593-6700.';
...
export function generateWelcomeResponse(language: 'en' | 'es'): string {
  const response = new MessagingResponse();

  const message = language === 'es'
    ? 'Hola! Soy el asistente virtual de la Ciudad de Doral. Como puedo ayudarle? Responda STOP para cancelar, HELP para ayuda.'
    : 'Hello! I am the City of Doral virtual assistant. How can I help you? Reply STOP to unsubscribe, HELP for info.';
